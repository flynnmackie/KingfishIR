...
def generate_launch_report(records, run_folder, hosts, out_path):
    """Build an HTML launch report (summary + per-host per-action detail)."""
    host_meta = {h.ip: h for h in hosts}
    ...
    # actions that represent actual launcher activity (not session/remote-command noise)
    LAUNCH_ACTIONS = ("launch run", "launch push", "launch pull", "launch collect",
                      "launch extract", "launch archive", "launch cleanup",
                      "sysmon", "sysmon push", "sysmon install", "sysmon verify",
                      "sysmon deployed", "velo", "velo push", "velo install",
                      "velo verify", "velo deployed", "deploy")
    ...
    for ip, recs in by_host.items():
        ...
        launch_recs = [r for r in recs if r.action in LAUNCH_ACTIONS]
        h_errors = sum(1 for r in launch_recs if r.outcome == "error")
        tot_actions += len(launch_recs)
        tot_errors += h_errors

        action_rows = []
        for r in launch_recs:
            cls = "err" if r.outcome == "error" else "ok"
            action_rows.append(
                f"<tr><td>{_esc(r.action)}</td>"
                f"<td>{_esc(r.artefact)}</td>"
                f"<td class='{cls}'>{_esc(r.outcome)}</td>"
                f"<td class='detail'>{_esc(r.detail)}</td></tr>")

        # what was deployed/run, distinct
        done = "; ".join(sorted({r.action for r in launch_recs})) or "—"
        summary_rows.append(
            f"<tr><td class='mono'>{_esc(ip)}</td><td>{_esc(hostname)}</td>"
            f"<td>{_esc(os_name)}</td><td>{_esc(done)}</td>"
            f"<td class='{'err' if h_errors else 'ok'}'>{h_errors}</td></tr>")

        action_table = ("".join(action_rows) if action_rows
                        else "<tr><td colspan='4' class='detail'>No launcher actions.</td></tr>")
        host_sections.append(f"""
  <details>
    <summary><b>{_esc(ip)}</b> {_esc(hostname)} — {len(launch_recs)} action(s),
      {h_errors} error(s)</summary>
    <table class="detail-table">
      <tr><th>Action</th><th>Target</th><th>Outcome</th><th>Detail</th></tr>
      {action_table}
    </table>
  </details>""")

    generated = datetime.now(timezone.utc).isoformat(timespec="seconds")
    err_class = "err" if tot_errors else "ok"
...
<div class="wrap">
  <div class="cards">
    ...
    <div class="card"><div class="n {err_class}">{tot_errors}</div><div class="l">Errors</div></div>
  </div>
